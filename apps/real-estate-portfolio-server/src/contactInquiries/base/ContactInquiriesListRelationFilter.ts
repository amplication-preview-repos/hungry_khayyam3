/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ContactInquiriesWhereInput } from "./ContactInquiriesWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class ContactInquiriesListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => ContactInquiriesWhereInput,
  })
  @ValidateNested()
  @Type(() => ContactInquiriesWhereInput)
  @IsOptional()
  @Field(() => ContactInquiriesWhereInput, {
    nullable: true,
  })
  every?: ContactInquiriesWhereInput;

  @ApiProperty({
    required: false,
    type: () => ContactInquiriesWhereInput,
  })
  @ValidateNested()
  @Type(() => ContactInquiriesWhereInput)
  @IsOptional()
  @Field(() => ContactInquiriesWhereInput, {
    nullable: true,
  })
  some?: ContactInquiriesWhereInput;

  @ApiProperty({
    required: false,
    type: () => ContactInquiriesWhereInput,
  })
  @ValidateNested()
  @Type(() => ContactInquiriesWhereInput)
  @IsOptional()
  @Field(() => ContactInquiriesWhereInput, {
    nullable: true,
  })
  none?: ContactInquiriesWhereInput;
}
export { ContactInquiriesListRelationFilter as ContactInquiriesListRelationFilter };
